...
public class MessageBroker {
    ...
    public String formatUserMessage(Session session) {
        return Json.createObjectBuilder()
                .add("id", session.getUserProperties().get(FieldValues.SESSION_USERID).toString())
                .add("name", session.getUserProperties().get(FieldValues.SESSION_USERNAME).toString())
                .add("imageUrl", "https://ui-avatars.com/api/?name=" + session.getUserProperties().get(FieldValues.SESSION_USERNAME).toString() + "&background=0D8ABC&color=fff")
                .build()
                .toString();
    }
}
